...
1. **Real settlement rail (gated on funding + a custom authorized-venue integration).** Replace the labeled `simulated` seam with real onchain settlement under a self-custody wallet. **This cannot run on testnet** — B20 tokenized stocks and their Chainlink feeds are Base-mainnet-only (verified), and the standard swap tooling (0x, CoW, sugar-sdk/Base MCP) does **not** route these stock tokens (verified `USDC→NVDAc` → `token not found`), so it requires a custom, pool-direct integration against an authorized Aerodrome venue plus real mainnet capital (gas + input token). This is the single change that flips Sovereign Index from "honest simulation" to "product."
