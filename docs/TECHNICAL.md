...
- **Onchain settlement is not wired** — B20 transfer policy is allow-list/authorized-venue only; general DEX aggregators (0x, 1inch, CoW) return "no route". Execution is paper at the live Chainlink price, labeled `simulated` everywhere. This is the deliberate, disclosed seam; a real authorized-venue router slots in at `engine.js` → the single execution call. Not a hidden gap.
